<form method="post" autocomplete="off" action="{{route('downloads.update', $download->id)}}" enctype="multipart/form-data">
    ...
    <div class="col-md-12">
        <div class="form-group {{ $errors->has('document_file') ? ' has-error' : '' }}">
            ...
            <input type="file" name="document_file" class="form-control dropify" data-default-file="/frontend/images/{{ $download->document_file }}" data-allowed-file-extensions="png jpg jpeg PNG JPG JPEG pdf PDF doc dot wdk docx xlsb xlsx ppt pptx"
                required>
        </div>
    </div>
