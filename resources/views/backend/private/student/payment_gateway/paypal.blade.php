...
<div class="invoice-col text-center">
    ...
    <div class="payment-btn-container hidden-print" align="center">
        <p>Esewa ID: {{get_option('esewa_id')}}<br>
            Payee Name: {{ get_option('esewa_payee') }}<br>
            (Please Mention domain name/Reference Number on remarks)<br>Reference Number: {{ strrev ( date('Ym', strtotime( $invoice->due_date ))) }}{{ $invoice->id }}</p>
    </div>
