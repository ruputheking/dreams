...
<div class="row">
	<div class="col-md-10">
		<div class="panel panel-default">
			<div class="panel-heading panel-title">Permission Control</div>

			<div class="panel-body">
				<form method="post" class="validate" autocomplete="off" action="{{ url('dashboard/permission/store') }}">
					{{ csrf_field() }}
					...
					<div class="form-group params-panel" style="background: #bdc3c7;">
						...
						<select class="form-control select2" onchange="showRole(this);" name="role_id" required>
							...
							{{ create_option("roles","id","display_name",$role_id) }}
						</select>
					</div>
							...
							<div id="collapse-{{ explode("\\",$key)[3] }}" class="panel-collapse collapse">
								<div class="panel-body">
									<table>
										...
										<tr>
											<td><label class="c-container">{{ str_replace("index","list",$name) }}<input name="permissions[]" value="{{ $name }}" type="checkbox"
														{{ array_search($name,$permission_list) !== FALSE ? "checked" : "" }}><span class="checkmark"></span></td>
										</tr>
										@endforeach
									</table>
								</div>
