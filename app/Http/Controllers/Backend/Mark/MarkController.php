...
class MarkController extends Controller
{
    ...
    {
		$class = $class;
        $students = Student::join('users','users.id','=','students.author_id')
					...
					->select('users.*','courses.title','sections.section_name','students.id as id','courses.id as class_id', 'students.student_name', 'students.photo')
	...
	public function view_marks($student_id='',$class_id=''){
		...
		$subjects = Subject::where("course_id",$class_id)->get();
    ...
    public function store(Request $request)
    {
		...
		if(! $request->ajax()){
		   return redirect('/marks/create')->with('success','Saved Sucessfully');
        }else{
   ...
   public function student_ranks($class =""){
	...
	if($class !="" ){
	   $class_id = $class;
	   $session = get_option('academic_years');

	   $students = DB::select("SELECT marks.student_id,students.student_name,users.photo,courses.title,sections.section_name,
	   IFNULL(SUM(mark_details.mark_value),0) as total_marks FROM marks,mark_details,exams,students,users,student_sessions,courses,sections
	   WHERE marks.id=mark_details.mark_id AND marks.exam_id=exams.id AND marks.student_id=students.id AND students.author_id=users.id AND
	   students.id=student_sessions.student_id AND student_sessions.course_id=courses.id AND student_sessions.section_id=sections.id
