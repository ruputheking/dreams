...
class ClassRoutine extends Model
{
	...
	public static function RoutineList($class_id = ""){
		...
		$filter = $class_id !="" ? "WHERE courses.id='$class_id'" : "";

		$routine_list = DB::select("SELECT courses.title,sections.section_name, courses.id as c_id, sections.id as s_id, class_routines.id as r_id
		FROM courses INNER JOIN sections ON courses.id=sections.course_id LEFT JOIN class_routines ON sections.id=class_routines.section_id $filter GROUP by sections.id");
		return $routine_list;
	}
	...
	public static function getRoutine($class="",$section=""){
		...
		FROM class_days JOIN subjects WHERE class_days.is_active=1 AND subjects.course_id='$class') as subject LEFT JOIN class_routines ON subject.s_id=class_routines.subject_id AND subject.class_day=class_routines.day AND
    ...
    public static function getTeacherRoutine($teacher_id=""){
		...
		$subjects = DB::select("SELECT subjects.subject_name,teachers.name,class_routines.*,courses.title,sections.section_name
		...
		ON assign_subjects.subject_id=class_routines.subject_id AND class_routines.section_id=assign_subjects.section_id JOIN sections ON class_routines.section_id=sections.id JOIN courses ON sections.course_id=courses.id
